 ...
 */
public class OtherDBFluteInitializer {

    // ===================================================================================
    //                                                                          Definition
    //                                                                          ==========
    /** Log instance. */
    private static final Log _log = LogFactory.getLog(OtherDBFluteInitializer.class);
     ...
     */
    public OtherDBFluteInitializer(javax.sql.DataSource dataSource) {
        if (dataSource == null) {
    ...
    protected void standBy() {
        if (!OtherDBFluteConfig.getInstance().isLocked()) {
            OtherDBFluteConfig.getInstance().lock();
        }
    }
    ...
    protected void handleSqlLogRegistry() { // for S2Container
        if (OtherDBFluteConfig.getInstance().isUseSqlLogRegistry()) {
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        final OtherDBFluteConfig config = OtherDBFluteConfig.getInstance();
        ...
        if (dataSourceFqcn.startsWith("org.apache.commons.dbcp.")) {
            config.unlock();
            config.setDataSourceHandler(new OtherDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }
    ...
    protected boolean isCurrentDBDef(DBDef currentDBDef) {
        return OtherDBCurrent.getInstance().isCurrentDBDef(currentDBDef);
    }
