package jp.co.np.p2netex.sample2.allcommon.s2dao.internal.sqlparser;
...
import jp.co.np.p2netex.sample2.allcommon.cbean.Sample2MapParameterBean;
import jp.co.np.p2netex.sample2.allcommon.cbean.coption.Sample2LikeSearchOption;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2BindVariableCommentNotFoundPropertyException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2BindVariableParameterNullValueException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2EmbeddedValueCommentNotFoundPropertyException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2EmbeddedValueParameterNullValueException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2EndCommentNotFoundException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2IfCommentConditionNotFoundException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2IfCommentNotBooleanResultException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2IfCommentWrongExpressionException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2RequiredOptionNotFoundException;
import jp.co.np.p2netex.sample2.allcommon.s2dao.internal.util.Sample2InternalBindVariableUtil;
 ...
 */
public class Sample2InternalSqlParser {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected Sample2InternalSqlTokenizer tokenizer;
    ...
    //                                                                         Constructor
    //                                                                         ===========
    public Sample2InternalSqlParser(String sql) {
        sql = sql.trim();
        if (sql.endsWith(";")) {
            sql = sql.substring(0, sql.length() - 1);
        }
        specifiedSql = sql;// Extension!
        tokenizer = new Sample2InternalSqlTokenizer(sql);
    }
    ...
    public Node parse() {
        ...
        while (Sample2InternalSqlTokenizer.EOF != tokenizer.next()) {
            parseToken();
        }
        return pop();
    }
    ...
    protected void parseToken() {
        ...
        case Sample2InternalSqlTokenizer.SQL:
            parseSql();
            break;
        case Sample2InternalSqlTokenizer.COMMENT:
            parseComment();
            break;
        case Sample2InternalSqlTokenizer.ELSE:
            parseElse();
            break;
        case Sample2InternalSqlTokenizer.BIND_VARIABLE:
            parseBindVariable();
            break;
        }
    }
    ...
    protected void parseSql() {
        ...
        if ((node instanceof InternalIfNode || node instanceof ElseNode) && node.getChildSize() == 0) {
            ...
            Sample2InternalSqlTokenizer st = new Sample2InternalSqlTokenizer(sql);
            st.skipWhitespace();
    ...
    protected void throwIfCommentConditionNotFoundException() {
        ...
        msg = msg + "* * * * * * * * * */";
        throw new Sample2IfCommentConditionNotFoundException(msg);
    }
    ...
    protected void parseEnd() {
        while (Sample2InternalSqlTokenizer.EOF != tokenizer.next()) {
            if (tokenizer.getTokenType() == Sample2InternalSqlTokenizer.COMMENT && isEndComment(tokenizer.getToken())) {
                pop();
                return;
            }
            parseToken();
    ...
    protected void throwEndCommentNotFoundException() {
        ...
        msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
        msg = msg + "* * * * * * * * * */";
        throw new Sample2EndCommentNotFoundException(msg);
    }
    ...
    protected static class InternalValueAndTypeSetuper {
        ...
        protected void setupValueAndType(InternalValueAndType valueAndType) {
            ...
            Sample2LikeSearchOption likeSearchOption = null;
            ...
            for (int pos = 1; pos < _names.length; ++pos) {
                ...
                if (pos == 1 && Sample2MapParameterBean.class.isAssignableFrom(clazz)) {
                    final Sample2MapParameterBean pmb = (Sample2MapParameterBean)value;
        ...
        // for OutsideSql
        protected boolean isLastLoop4LikeSearch(int pos, Sample2LikeSearchOption likeSearchOption) {
        ...
        // for OutsideSql
        protected Sample2LikeSearchOption getLikeSearchOption(BeanDesc beanDesc, String currentName, Object resourceBean) {
            final PropertyDesc pb = beanDesc.getPropertyDesc(currentName + "InternalLikeSearchOption");
			final Sample2LikeSearchOption option = (Sample2LikeSearchOption) pb.getValue(resourceBean);
			if (option == null) {
        ...
        // for OutsideSql
        protected void throwOutsideSqlLikeSearchOptionSplitUnsupportedException(Sample2LikeSearchOption option, Object resourceBean, String currentName) {
            String msg = "Look! Read the message below." + getLineSeparator();
            msg = msg + "/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" + getLineSeparator();
        ...
        protected String initCap(String name) {
		    return Sample2SimpleStringUtil.initCap(name);
        }
        ...
        protected void throwBindOrEmbeddedCommentNotFoundPropertyException(String expression, Class<?> targetType, String notFoundProperty, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new Sample2BindVariableCommentNotFoundPropertyException(msg);
            } else {
                throw new Sample2EmbeddedValueCommentNotFoundPropertyException(msg);
            }
        }
	...
	protected static class ParameterCommentExceptionProvider {
        public static void throwBindOrEmbeddedParameterNullValueException(String expression, Class<?> targetType, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new Sample2BindVariableParameterNullValueException(msg);
            } else {
                throw new Sample2EmbeddedValueParameterNullValueException(msg);
            }
        }
    ...
    protected static class InternalIfNode extends ContainerNode {
        ...
        protected void throwIfCommentWrongExpressionException(String expression, RuntimeException cause, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new Sample2IfCommentWrongExpressionException(msg, cause);
        }
        ...
        protected void throwIfCommentNotBooleanResultException(String expression, Object result, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new Sample2IfCommentNotBooleanResultException(msg);
        }
	...
	public static String convertTwoWaySql2DisplaySql(String twoWaySql, String[] argNames, Class<?>[] argTypes, Object[] args) {
		...
		{
            final Sample2InternalSqlParser parser = new Sample2InternalSqlParser(twoWaySql);
            final Node node = parser.parse();
            final Sample2InternalCommandContextCreator creator = new Sample2InternalCommandContextCreator(argNames, argTypes);
            context = creator.createCommandContext(args);
            node.accept(context);
		}
        final String preparedSql = context.getSql();
		return Sample2InternalBindVariableUtil.getCompleteSql(preparedSql, context.getBindVariables());
	}
}
