...
import jp.co.np.p2netex.sample2.allcommon.Sample2QLog;
...
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2EntityAlreadyExistsException;
import jp.co.np.p2netex.sample2.allcommon.exception.Sample2SQLFailureException;
 ...
 */
public class Sample2InternalBasicHandler {
    ...
    //                                                                         Constructor
    //                                                                         ===========
    public Sample2InternalBasicHandler(DataSource ds, StatementFactory statementFactory) {
        setDataSource(ds);
        setStatementFactory(statementFactory);
    }

    public Sample2InternalBasicHandler(DataSource ds, String sql, StatementFactory statementFactory) {
        setDataSource(ds);
		setSql(sql);
        setStatementFactory(statementFactory);
    }
    ...
    protected String getCompleteSql(Object[] args) {
        return Sample2InternalBindVariableUtil.getCompleteSql(sql, args);
    }

    protected String getBindVariableText(Object bindVariable) {
        return Sample2InternalBindVariableUtil.getBindVariableText(bindVariable);
    }
    ...
    protected void logSql(Object[] args, Class<?>[] argTypes) {
        if (Sample2QLog.isLogEnabled() || Sample2InternalSqlLogRegistry.existsSqlLogRegistry()) {
            final String completeSql = getCompleteSql(args);
			if (isContainsLineSeparatorInSql()) {
                Sample2QLog.log(getLineSeparator() + completeSql);
			} else {
                Sample2QLog.log(completeSql);
			}
		    if (Sample2InternalSqlLogRegistry.existsSqlLogRegistry()) {
			    final Object sqlLogRegistry = Sample2InternalSqlLogRegistry.findContainerSqlLogRegistry();
				if (sqlLogRegistry != null) {
				    Sample2InternalSqlLogRegistry.push(getSql(), completeSql, args, argTypes, sqlLogRegistry);
				}
			}
        }
    }
    ...
    public static class SQLExceptionHandler {
	    ...
	    protected boolean isUniqueConstraintException(SQLException e) {
	        Sample2DBFluteConfig.UniqueConstraintDeterminator determinator = getUniqueConstraintDeterminator();
	        if (determinator != null) {
	            return determinator.isUniqueConstraintException(e);
	        }
	        return Sample2ConditionBeanContext.isUniqueConstraintException(extractSQLState(e), e.getErrorCode());
	    }
	
	    protected Sample2DBFluteConfig.UniqueConstraintDeterminator getUniqueConstraintDeterminator() {
	        return Sample2DBFluteConfig.getInstance().getUniqueConstraintDeterminator();
	    }
	
	    protected boolean isSqlExceptionOldStyleHandling() {
	        return Sample2DBFluteConfig.getInstance().isSqlExceptionOldStyleHandling();
	    }
	    ...
	    protected void throwEntityAlreadyExistsException(SQLException e, Statement statement, Object resource) {
	        ...
	        msg = msg + "* * * * * * * * * */";
	        throw new Sample2EntityAlreadyExistsException(msg, e);
	    }
	    ...
	    protected void throwSQLFailureException(SQLException e, Statement statement, Object resource) {
            ...
            msg = msg + "* * * * * * * * * */";
            throw new Sample2SQLFailureException(msg, e);
        }
