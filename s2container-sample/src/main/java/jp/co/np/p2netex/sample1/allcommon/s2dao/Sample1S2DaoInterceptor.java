...
import jp.co.np.p2netex.sample1.allcommon.Sample1XLog;
...
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1EntityAlreadyUpdatedException;
 ...
 */
public class Sample1S2DaoInterceptor extends org.seasar.framework.aop.interceptors.AbstractInterceptor {
    ...
    /** Log instance. */
    private static final org.apache.commons.logging.Log _log = org.apache.commons.logging.LogFactory.getLog(Sample1S2DaoInterceptor.class);
     ...
     */
    public Sample1S2DaoInterceptor(DaoMetaDataFactory daoMetaDataFactory) {
    ...
    protected boolean isLogEnabled() {
        return Sample1XLog.isLogEnabled();
    }
    ...
    protected Object dispatchInvoking(MethodInvocation invocation) throws Throwable {
        ...
        // - - - - - - - - - - - - -
        final Sample1ConditionBean cb = preprocessConditionBean(invocation);
        ...
        try {
            ...
            if (isLogEnabled()) {
                ...
                sb.append("  args   = " + Sample1TraceViewUtil.convertObjectArrayToStringView(invocation.getArguments()));
                log(sb.toString());
            }
            if (e.getClass().equals(NotSingleRowUpdatedRuntimeException.class)) {
                throw new Sample1EntityAlreadyUpdatedException((NotSingleRowUpdatedRuntimeException)e);
            }
            throw e;
        } finally {
    ...
    protected void initializeSqlCommand(MethodInvocation invocation) {
        ...
        if (Sample1OutsideSqlDao.class.isAssignableFrom(targetType)) {
            return;// Do nothing!
        } else {
    ...
    protected String generateSpecifiedOutsideSqlUniqueKey(MethodInvocation invocation) {
        ...
        final Sample1OutsideSqlOption option = (Sample1OutsideSqlOption)args[2];
        Object resultTypeSpecification = null;
        if (args.length > 3) {
            resultTypeSpecification = args[3];
        }
        return Sample1OutsideSqlContext.generateSpecifiedOutsideSqlUniqueKey(invocation.getMethod().getName(), path, pmb, option, resultTypeSpecification);
    }
    ...
    protected void logInvocation(MethodInvocation invocation) {
        ...
        if (isSpecifiedOutsideSql(invocation)) {
		    final Sample1OutsideSqlContext outsideSqlContext = getOutsideSqlContext();
    ...
    protected String removeBasePrefixFromSimpleClassName(String simpleClassName) {
        if (!simpleClassName.startsWith("Sample1Bs")) {
            return simpleClassName;
        }
        final int prefixLength = "Sample1Bs".length();
        ...
        return "Sample1" + simpleClassName.substring(prefixLength);
    }
    ...
    protected void logSqlCommand(MethodInvocation invocation, SqlCommand cmd, long beforeCmd, long afterCmd) {
        log("SqlCommand Initialization Cost: [" + Sample1TraceViewUtil.convertToPerformanceView(afterCmd - beforeCmd) + "]");
    }
    ...
    protected void assertRetType(Class<?> retType, Object ret) {
        ...
        } else if (Sample1Entity.class.isAssignableFrom(retType)) {
            if (ret != null && !(ret instanceof Sample1Entity)) {
                String msg = "The retType is difference from actual return: ";
                msg = msg + "retType=" + retType + " ret.getClass()=" + ret.getClass() + " ref=" + ret;
                throw new IllegalStateException(msg);
            }
    ...
    protected void logReturn(org.aopalliance.intercept.MethodInvocation invocation, Class<?> retType, Object ret, long before, long after) throws Throwable {
        try {
            final String daoResultPrefix = "===========/ [" + Sample1TraceViewUtil.convertToPerformanceView(after - before) + " - ";
            if (java.util.List.class.isAssignableFrom(retType)) {
                if (ret == null) {
                ...
                if (ret == null) {
                    ...
                    final Sample1Entity entity = (Sample1Entity) ret;
    ...
    protected void preprocessOutsideSql(MethodInvocation invocation) {
        final Class<jp.co.np.p2netex.sample1.allcommon.annotation.OutsideSql> outsideSqlType = jp.co.np.p2netex.sample1.allcommon.annotation.OutsideSql.class;
        final jp.co.np.p2netex.sample1.allcommon.annotation.OutsideSql outsideSql = invocation.getMethod().getAnnotation(outsideSqlType);
        ...
        if (outsideSql != null && (outsideSql.dynamicBinding() || outsideSql.offsetByCursor() || outsideSql.offsetByCursor() || outsideSql.limitByCursor())) {
            final Sample1OutsideSqlContext outsideSqlContext = new Sample1OutsideSqlContext();
            ...
            Sample1OutsideSqlContext.setOutsideSqlContextOnThread(outsideSqlContext);
            ...
            if (Sample1FetchNarrowingBeanContext.isTheTypeFetchNarrowingBean(args[0].getClass())) {
                Sample1FetchNarrowingBeanContext.setFetchNarrowingBeanOnThread((Sample1FetchNarrowingBean)args[0]);
            }
            return;
        }
    ...
    protected boolean isSpecifiedOutsideSql(MethodInvocation invocation) {
        return Sample1OutsideSqlDao.class.isAssignableFrom(getTargetClass(invocation));
    }
    ...
    protected void setupOutsideSqlContextSelect(MethodInvocation invocation) {
        ...
        final Sample1OutsideSqlOption option = getOutsideSqlOption(args);
    ...
    protected Sample1OutsideSqlOption getOutsideSqlOption(Object[] args) {
        return (Sample1OutsideSqlOption)args[2];
    }

    protected void setupOutsideSqlFetchNarrowingBean(Object pmb, Sample1OutsideSqlOption option) {
        if (pmb == null || !Sample1FetchNarrowingBeanContext.isTheTypeFetchNarrowingBean(pmb.getClass())) {
            return;
        }
        final Sample1FetchNarrowingBean fetchNarrowingBean = (Sample1FetchNarrowingBean)pmb;
        if (option.isManualPaging()) {
            fetchNarrowingBean.ignoreFetchNarrowing();
        }
        Sample1FetchNarrowingBeanContext.setFetchNarrowingBeanOnThread(fetchNarrowingBean);
    }
     ...
     */
    protected Sample1ConditionBean preprocessConditionBean(MethodInvocation invocation) {
        final Sample1OutsideSqlContext outsideSqlContext = getOutsideSqlContext();
        if (outsideSqlContext != null) {
            return null; // Because it has already finished setting up fetchNarrowingBean for outsideSql here.
        }
        
        final Sample1ConditionBean cb;
        {
            final Object[] args = invocation.getArguments();
            if (args == null || args.length == 0) {
                return null;
            ...
            if (!Sample1ConditionBeanContext.isTheTypeConditionBean(arg0.getClass())) {// The argument is not condition-bean...
                if (Sample1FetchNarrowingBeanContext.isTheTypeFetchNarrowingBean(arg0.getClass()) && !isSelectCountIgnoreFetchScopeMethod(invocation)) {
                    // Fetch-narrowing-bean and Not select count!
                    Sample1FetchNarrowingBeanContext.setFetchNarrowingBeanOnThread((Sample1FetchNarrowingBean)arg0);
                }
                return null;
            }
            cb = (Sample1ConditionBean)arg0;
        }

        if (isSelectCountIgnoreFetchScopeMethod(invocation)) {
            cb.xsetupSelectCountIgnoreFetchScope();
        } else {
            Sample1FetchNarrowingBeanContext.setFetchNarrowingBeanOnThread(cb);
        }

        Sample1ConditionBeanContext.setConditionBeanOnThread(cb);
        return cb;
    }
     ...
     */
    protected void postprocessConditionBean(MethodInvocation invocation, Sample1ConditionBean cb) {
        if (cb == null) {
            return;
        }
        if (isSelectCountIgnoreFetchScopeMethod(invocation)) {
    ...
    protected void clearThreadLocal() {
        if (Sample1OutsideSqlContext.isExistOutsideSqlContextOnThread()) {
            Sample1OutsideSqlContext.clearOutsideSqlContextOnThread();
        }
        if (Sample1FetchNarrowingBeanContext.isExistFetchNarrowingBeanOnThread()) {
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            Sample1FetchNarrowingBeanContext.getFetchNarrowingBeanOnThread().restoreIgnoredFetchNarrowing();

            Sample1FetchNarrowingBeanContext.clearFetchNarrowingBeanOnThread();
        }
        if (Sample1ConditionBeanContext.isExistConditionBeanOnThread()) {
            Sample1ConditionBeanContext.clearConditionBeanOnThread();
        }
        if (Sample1InternalMapContext.isExistInternalMapContextOnThread()) {
            Sample1InternalMapContext.clearInternalMapContextOnThread();
        }
    }
    ...
    //                                                                      Context Helper
    //                                                                      ==============
    protected Sample1OutsideSqlContext getOutsideSqlContext() {
        if (!Sample1OutsideSqlContext.isExistOutsideSqlContextOnThread()) {
            return null;
        }
        return Sample1OutsideSqlContext.getOutsideSqlContextOnThread();
    }
