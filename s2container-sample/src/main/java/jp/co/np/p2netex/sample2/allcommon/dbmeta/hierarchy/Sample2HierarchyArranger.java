...
public class Sample2HierarchyArranger<LOCAL_ENTITY extends Sample2Entity> {
     ...
     */
    public List<LOCAL_ENTITY> arrangeHierarchy(Sample2HierarchyRequest<LOCAL_ENTITY> request) {
        final List<LOCAL_ENTITY> localTableList = new ArrayList<LOCAL_ENTITY>();
        final Map<String, Sample2Entity> alreadyRegisteredEntityMap = new HashMap<String, Sample2Entity>();

        // ============
        // Record Loop!
        // ============
        final Sample2HierarchySourceIterator sourceIterator = request.getSourceIterator();
        while (sourceIterator.hasNext()) {
            final Sample2HierarchySourceRow sourceRow = sourceIterator.next();
            ...
            if (alreadyRegisteredEntityMap.containsKey(alreadyRegisteredKey)) {
                final Sample2Entity localEntity = alreadyRegisteredEntityMap.get(alreadyRegisteredKey);
        ...
        for (String key : alreadyRegisteredEntityKeySet) {
            final Sample2Entity currentRegisteredEntity = alreadyRegisteredEntityMap.get(key);
    ...
    protected String buildTopAlreadyRegisteredKey(Map<String, Object> primaryKeyMap) {
        return Sample2HierarchyRequestElement.TOP_KEY + ":" + primaryKeyMap;
    }
    ...
    protected void doColumnLoop(TopInfo<LOCAL_ENTITY> topInfo) {
        final Sample2HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Sample2Entity localEntity = topInfo.getLocalEntity();
        final List<Sample2HierarchyRequestElement> requestElementList = request.getRequestElementList();

        // ============
        // Column Loop!
        // ============
        for (Sample2HierarchyRequestElement requestElement : requestElementList) {
            ...
            if (relationPropertyNameList == null || relationPropertyNameList.isEmpty()) {
                final Sample2HierarchySourceColumn sourceColumn = requestElement.getSourceColumnInfo();
                final Sample2HierarchySourceRow sourceRow = topInfo.getSourceRow();
                final Object sourceColumnValue = extractColumnValueFromSource(sourceRow, sourceColumn);
                final Sample2ColumnInfo destinationColumnInfo = requestElement.getDestinationColumnInfo();
     ...
     */
    protected void doRelationLoop(TopInfo<LOCAL_ENTITY> topInfo, Sample2HierarchyRequestElement requestElement, java.util.List<String> relationPropNameList) {
        final Sample2HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Sample2Entity> alreadyRegisteredEntityMap = topInfo.getAlreadyRegisteredEntityMap();

        // Temporary variables for local
        Sample2Entity localEntity = topInfo.getLocalEntity();// as Default
        Sample2DBMeta localDBMeta = request.getDestinationDBMeta();// as Default
        String localRelationPath = Sample2HierarchyRequestElement.TOP_KEY;// as Default
        ...
        for (String relationPropName : relationPropNameList) {
            ...
            final Sample2RelationInfo relationInfo = localDBMeta.findRelationInfo(relationPropName);
            ...
            if (!relationInfo.isReferrer()) {
                ...
                // Foreign
                // =======
                final Sample2ForeignInfo foreignInfo = localDBMeta.findForeignInfo(relationPropName);
                ...
                if (!alreadyRegisteredEntityMap.containsKey(alreadyRegisteredEntityKey)) {
                    ...
                    // - - - - - - - - - - - - - - - - - - - - - - -
                    final Sample2Entity foreignEntity = foreignInfo.getForeignDBMeta().newEntity();
                    ...
                    // - - - - - - - - - - - - - -
                    final Sample2Entity foreignEntity = alreadyRegisteredEntityMap.get(alreadyRegisteredEntityKey);
                    injectForeignEntity(localEntity, foreignPropName, foreignEntity);
                }
            } else {
                // =======
                // Referrer
                // =======
                final Sample2ReferrerInfo referrerInfo = localDBMeta.findReferrerInfo(relationPropName);

                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // Extract referrer list from current local entity and initialize it if needs.
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                List<Sample2Entity> referrerList = extractReferrerList(localEntity, referrerInfo);
                ...
                if (!alreadyRegisteredEntityMap.containsKey(alreadyRegisteredEntityKey)) {
                    ...
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    final Sample2Entity referrerEntity = relationInfo.getTargetDBMeta().newEntity();
     ...
     * @param primaryKeyMap The map of primary key. (NotNull)
     */
    protected void doLastLoopInjection(TopInfo<LOCAL_ENTITY> topInfo, Sample2HierarchyRequestElement requestElement,
                                       Sample2Entity localEntity, Map<String, Object> primaryKeyMap) {
        final Sample2ColumnInfo destinationColumnInfo = requestElement.getDestinationColumnInfo();
        if (!primaryKeyMap.containsKey(destinationColumnInfo.getColumnDbName())) {// The column is primary key!
            final Sample2HierarchySourceRow sourceRow = topInfo.getSourceRow();
            final Sample2HierarchySourceColumn sourceColumnInfo = requestElement.getSourceColumnInfo();
    ...
    protected Map<String, Object> extractTopPrimaryKeyMapFromSource(TopInfo<LOCAL_ENTITY> topInfo) {
        return extractPrimaryKeyMapFromSource(topInfo, Sample2HierarchyRequestElement.TOP_KEY);
    }

    protected Map<String, Object> extractPrimaryKeyMapFromSource(TopInfo<LOCAL_ENTITY> topInfo, String relationPath) {
        final Sample2HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Sample2HierarchySourceRow sourceRow = topInfo.getSourceRow();
        final java.util.List<Sample2HierarchyRequestElement> primaryKeyElement = request.findPrimaryKeyElement(relationPath);
        final java.util.Map<String, Object> primaryKeyMap = new java.util.LinkedHashMap<String, Object>();
        for (Sample2HierarchyRequestElement element : primaryKeyElement) {
            final Sample2HierarchySourceColumn sourcePrimaryKey = element.getSourceColumnInfo();
    ...
    protected Object extractColumnValueFromSource(Sample2HierarchySourceRow sourceRow,
            Sample2HierarchySourceColumn sourceColumn) {
        return sourceRow.extractColumnValue(sourceColumn);
    }

    @SuppressWarnings("unchecked")
    protected java.util.List<Sample2Entity> extractReferrerList(Sample2Entity entity, Sample2ReferrerInfo referrerInfo) {
        return (java.util.List<Sample2Entity>) invoke(referrerInfo.findGetter(), entity, new Object[] {});
    }
     ...
     */
    protected void injectColumnValueToDestinationIfNotNull(Sample2Entity entity, Sample2ColumnInfo columnInfo, final Object columnValue) {
        if (columnValue != null) {
            injectColumnValueToDestination(entity, columnInfo.getColumnDbName(), columnValue);
        }
    }

    protected void injectColumnValueToDestination(Sample2Entity entity, String columnDbName, final Object columnValue) {
        if (columnValue == null) {
    ...
    protected void injectColumnValueMapToDestination(Sample2Entity entity, final Map<String, Object> columnValueMap) {
    ...
    protected void injectForeignEntity(Sample2Entity entity, String foreignPropName, Sample2Entity foreignEntity) {
    ...
    protected void injectReferrerList(Sample2Entity entity, Sample2ReferrerInfo referrerInfo, java.util.List<Sample2Entity> referrerList) {
        invoke(referrerInfo.findSetter(), entity, new Object[] { referrerList });
    }

    protected void injectForeignPrimaryKey(Sample2Entity foreignEntity, Map<String, Object> foreigPrimaryKeyMap) {
        injectColumnValueMapToDestination(foreignEntity, foreigPrimaryKeyMap);
    }

    protected void injectReferrerPrimaryKey(Sample2Entity referrerEntity, Map<String, Object> referrerPrimaryKeyMap) {
        injectColumnValueMapToDestination(referrerEntity, referrerPrimaryKeyMap);
    }

    protected void injectLocalForeignKey(TopInfo<LOCAL_ENTITY> topInfo, Sample2Entity localEntity, Sample2ForeignInfo foreignInfo, String foreignRelationPath) {
        final Sample2HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Object> foreignPrimaryKeyMap = extractPrimaryKeyMapFromSource(topInfo, foreignRelationPath);
        final List<Sample2HierarchyRequestElement> primaryKeyElementList = request.findPrimaryKeyElement(foreignRelationPath);
        final Map<String, Object> localForeignKeyMap = new HashMap<String, Object>();
        for (Sample2HierarchyRequestElement foreignElement : primaryKeyElementList) {
            final String foreignPrimaryKeyColumnName = foreignElement.getDestinationColumnInfo().getColumnDbName();
            final Sample2ColumnInfo localForeignKeyInfo = foreignInfo.findLocalByForeign(foreignPrimaryKeyColumnName);
    ...
    protected void injectReferrerForeignKey(TopInfo<LOCAL_ENTITY> topInfo, Sample2Entity referrerEntity,
            Sample2ReferrerInfo referrerInfo, String localRelationPath) {
        final Sample2HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Object> localPrimaryKeyMap = extractPrimaryKeyMapFromSource(topInfo, localRelationPath);
        final List<Sample2HierarchyRequestElement> primaryKeyElementList = request.findPrimaryKeyElement(localRelationPath);
        final Map<String, Object> referrerForeignKeyMap = new HashMap<String, Object>();
        for (Sample2HierarchyRequestElement localElement : primaryKeyElementList) {
            final String localPrimaryKeyName = localElement.getDestinationColumnInfo().getColumnDbName();
            final Sample2ColumnInfo referrerForeignKeyInfo = referrerInfo.findReferrerByLocal(localPrimaryKeyName);
    ...
    //                                                                         ===========
    @SuppressWarnings("unchecked")
    protected LOCAL_ENTITY newLocalEntity(final Sample2DBMeta destinationDBMeta) {
    ...
    //                                                                          Info Class
    //                                                                          ==========
    protected static class TopInfo<LOCAL_ENTITY extends Sample2Entity> {
        private Sample2HierarchyRequest<LOCAL_ENTITY> hierarchyRequest;

        private Sample2HierarchySourceRow sourceRow;

        private Sample2Entity localEntity;

        private Map<String, Sample2Entity> alreadyRegisteredEntityMap;

        public Sample2HierarchySourceRow getSourceRow() {
            return sourceRow;
        }

        public void setSourceRow(Sample2HierarchySourceRow sourceRow) {
            this.sourceRow = sourceRow;
        }

        public Sample2Entity getLocalEntity() {
            return localEntity;
        }

        public void setLocalEntity(Sample2Entity localEntity) {
            this.localEntity = localEntity;
        }

        public Map<String, Sample2Entity> getAlreadyRegisteredEntityMap() {
            return alreadyRegisteredEntityMap;
        }

        public void setAlreadyRegisteredEntityMap(Map<String, Sample2Entity> alreadyRegisteredEntityMap) {
            this.alreadyRegisteredEntityMap = alreadyRegisteredEntityMap;
        }

        public Sample2HierarchyRequest<LOCAL_ENTITY> getHierarchyRequest() {
            return hierarchyRequest;
        }

        public void setHierarchyRequest(Sample2HierarchyRequest<LOCAL_ENTITY> hierarchyRequest) {
