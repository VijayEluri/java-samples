 ...
 */
public class Sample1PagingInvoker<ENTITY> {
    ...
    //                                                                         Constructor
    //                                                                         ===========
    public Sample1PagingInvoker(String tableDbName) {
     ...
     */
    public Sample1PagingResultBean<ENTITY> invokePaging(Sample1PagingHandler<ENTITY> handler) {
        assertObjectNotNull("handler", handler);
        final Sample1PagingBean pagingBean = handler.getPagingBean();
        ...
        final Sample1PagingResultBean<ENTITY> rb = new Sample1ResultBeanBuilder<ENTITY>(_tableDbName).buildPagingResultBean(pagingBean, allRecordCount, selectedList);
        if (isNecessaryToReadPageAgain(rb)) {
            ...
            return new Sample1ResultBeanBuilder<ENTITY>(_tableDbName).buildPagingResultBean(pagingBean, reAllRecordCount, reSelectedList);
        } else {
            return rb;
        }
    }
     ...
     */
    protected boolean isNecessaryToReadPageAgain(Sample1PagingResultBean<ENTITY> rb) {
