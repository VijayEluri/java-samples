package jp.co.np.p2netex.sample1.allcommon.s2dao.internal.sqlparser;
...
import jp.co.np.p2netex.sample1.allcommon.cbean.Sample1MapParameterBean;
import jp.co.np.p2netex.sample1.allcommon.cbean.coption.Sample1LikeSearchOption;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1BindVariableCommentNotFoundPropertyException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1BindVariableParameterNullValueException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1EmbeddedValueCommentNotFoundPropertyException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1EmbeddedValueParameterNullValueException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1EndCommentNotFoundException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1IfCommentConditionNotFoundException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1IfCommentNotBooleanResultException;
import jp.co.np.p2netex.sample1.allcommon.exception.Sample1IfCommentWrongExpressionException;
 ...
 */
public class Sample1InternalSqlParser {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected Sample1InternalSqlTokenizer tokenizer;
    ...
    //                                                                         Constructor
    //                                                                         ===========
    public Sample1InternalSqlParser(String sql) {
        sql = sql.trim();
        if (sql.endsWith(";")) {
            sql = sql.substring(0, sql.length() - 1);
        }
        specifiedSql = sql;// Extension!
        tokenizer = new Sample1InternalSqlTokenizer(sql);
    }
    ...
    public Node parse() {
        ...
        while (Sample1InternalSqlTokenizer.EOF != tokenizer.next()) {
            parseToken();
        }
        return pop();
    }
    ...
    protected void parseToken() {
        ...
        case Sample1InternalSqlTokenizer.SQL:
            parseSql();
            break;
        case Sample1InternalSqlTokenizer.COMMENT:
            parseComment();
            break;
        case Sample1InternalSqlTokenizer.ELSE:
            parseElse();
            break;
        case Sample1InternalSqlTokenizer.BIND_VARIABLE:
            parseBindVariable();
            break;
        }
    }
    ...
    protected void parseSql() {
        ...
        if ((node instanceof InternalIfNode || node instanceof ElseNode) && node.getChildSize() == 0) {
            ...
            Sample1InternalSqlTokenizer st = new Sample1InternalSqlTokenizer(sql);
            st.skipWhitespace();
    ...
    protected void throwIfCommentConditionNotFoundException() {
        ...
        msg = msg + "* * * * * * * * * */";
        throw new Sample1IfCommentConditionNotFoundException(msg);
    }
    ...
    protected void parseEnd() {
        while (Sample1InternalSqlTokenizer.EOF != tokenizer.next()) {
            if (tokenizer.getTokenType() == Sample1InternalSqlTokenizer.COMMENT && isEndComment(tokenizer.getToken())) {
                pop();
                return;
            }
            parseToken();
    ...
    protected void throwEndCommentNotFoundException() {
        ...
        msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
        msg = msg + "* * * * * * * * * */";
        throw new Sample1EndCommentNotFoundException(msg);
    }
    ...
    protected static class InternalValueAndTypeSetuper {
        ...
        protected void setupValueAndType(InternalValueAndType valueAndType) {
            ...
            Sample1LikeSearchOption likeSearchOption = null;
            ...
            for (int pos = 1; pos < _names.length; ++pos) {
                ...
                if (pos == 1 && Sample1MapParameterBean.class.isAssignableFrom(clazz)) {
                    final Sample1MapParameterBean pmb = (Sample1MapParameterBean)value;
        ...
        // for OutsideSql
        protected boolean isLastLoop4LikeSearch(int pos, Sample1LikeSearchOption likeSearchOption) {
        ...
        // for OutsideSql
        protected Sample1LikeSearchOption getLikeSearchOption(BeanDesc beanDesc, String currentName, Object resourceBean) {
            final PropertyDesc pb = beanDesc.getPropertyDesc(currentName + "InternalLikeSearchOption");
			final Sample1LikeSearchOption option = (Sample1LikeSearchOption) pb.getValue(resourceBean);
			if (option == null) {
        ...
        // for OutsideSql
        protected void throwOutsideSqlLikeSearchOptionSplitUnsupportedException(Sample1LikeSearchOption option, Object resourceBean, String currentName) {
            String msg = "Look! Read the message below." + getLineSeparator();
            msg = msg + "/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" + getLineSeparator();
        ...
        protected String initCap(String name) {
		    return Sample1SimpleStringUtil.initCap(name);
        }
        ...
        protected void throwBindOrEmbeddedCommentNotFoundPropertyException(String expression, Class<?> targetType, String notFoundProperty, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new Sample1BindVariableCommentNotFoundPropertyException(msg);
            } else {
                throw new Sample1EmbeddedValueCommentNotFoundPropertyException(msg);
            }
        }
	...
	protected static class ParameterCommentExceptionProvider {
        public static void throwBindOrEmbeddedParameterNullValueException(String expression, Class<?> targetType, String specifiedSql, boolean bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new Sample1BindVariableParameterNullValueException(msg);
            } else {
                throw new Sample1EmbeddedValueParameterNullValueException(msg);
            }
        }
    ...
    protected static class InternalIfNode extends ContainerNode {
        ...
        protected void throwIfCommentWrongExpressionException(String expression, RuntimeException cause, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new Sample1IfCommentWrongExpressionException(msg, cause);
        }
        ...
        protected void throwIfCommentNotBooleanResultException(String expression, Object result, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + getLineSeparator() + specifiedSql + getLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new Sample1IfCommentNotBooleanResultException(msg);
        }
	...
	public static String convertTwoWaySql2DisplaySql(String twoWaySql, String[] argNames, Class<?>[] argTypes, Object[] args) {
		...
		{
            final Sample1InternalSqlParser parser = new Sample1InternalSqlParser(twoWaySql);
            final Node node = parser.parse();
            final Sample1InternalCommandContextCreator creator = new Sample1InternalCommandContextCreator(argNames, argTypes);
            context = creator.createCommandContext(args);
            node.accept(context);
		}
        final String preparedSql = context.getSql();
		return Sample1InternalBindVariableUtil.getCompleteSql(preparedSql, context.getBindVariables());
	}
}
