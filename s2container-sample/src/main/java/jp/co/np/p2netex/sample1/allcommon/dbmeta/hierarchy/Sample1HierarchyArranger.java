...
import jp.co.np.p2netex.sample1.allcommon.dbmeta.info.Sample1ForeignInfo;
import jp.co.np.p2netex.sample1.allcommon.dbmeta.info.Sample1ReferrerInfo;
...
public class Sample1HierarchyArranger<LOCAL_ENTITY extends Sample1Entity> {
     ...
     */
    public List<LOCAL_ENTITY> arrangeHierarchy(Sample1HierarchyRequest<LOCAL_ENTITY> request) {
        final List<LOCAL_ENTITY> localTableList = new ArrayList<LOCAL_ENTITY>();
        final Map<String, Sample1Entity> alreadyRegisteredEntityMap = new HashMap<String, Sample1Entity>();

        // ============
        // Record Loop!
        // ============
        final Sample1HierarchySourceIterator sourceIterator = request.getSourceIterator();
        while (sourceIterator.hasNext()) {
            final Sample1HierarchySourceRow sourceRow = sourceIterator.next();
            ...
            if (alreadyRegisteredEntityMap.containsKey(alreadyRegisteredKey)) {
                final Sample1Entity localEntity = alreadyRegisteredEntityMap.get(alreadyRegisteredKey);
        ...
        for (String key : alreadyRegisteredEntityKeySet) {
            final Sample1Entity currentRegisteredEntity = alreadyRegisteredEntityMap.get(key);
    ...
    protected String buildTopAlreadyRegisteredKey(Map<String, Object> primaryKeyMap) {
        return Sample1HierarchyRequestElement.TOP_KEY + ":" + primaryKeyMap;
    }
    ...
    protected void doColumnLoop(TopInfo<LOCAL_ENTITY> topInfo) {
        final Sample1HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Sample1Entity localEntity = topInfo.getLocalEntity();
        final List<Sample1HierarchyRequestElement> requestElementList = request.getRequestElementList();

        // ============
        // Column Loop!
        // ============
        for (Sample1HierarchyRequestElement requestElement : requestElementList) {
            ...
            if (relationPropertyNameList == null || relationPropertyNameList.isEmpty()) {
                final Sample1HierarchySourceColumn sourceColumn = requestElement.getSourceColumnInfo();
                final Sample1HierarchySourceRow sourceRow = topInfo.getSourceRow();
                final Object sourceColumnValue = extractColumnValueFromSource(sourceRow, sourceColumn);
                final Sample1ColumnInfo destinationColumnInfo = requestElement.getDestinationColumnInfo();
     ...
     */
    protected void doRelationLoop(TopInfo<LOCAL_ENTITY> topInfo, Sample1HierarchyRequestElement requestElement, java.util.List<String> relationPropNameList) {
        final Sample1HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Sample1Entity> alreadyRegisteredEntityMap = topInfo.getAlreadyRegisteredEntityMap();

        // Temporary variables for local
        Sample1Entity localEntity = topInfo.getLocalEntity();// as Default
        Sample1DBMeta localDBMeta = request.getDestinationDBMeta();// as Default
        String localRelationPath = Sample1HierarchyRequestElement.TOP_KEY;// as Default
        ...
        for (String relationPropName : relationPropNameList) {
            ...
            final Sample1RelationInfo relationInfo = localDBMeta.findRelationInfo(relationPropName);
            ...
            if (!relationInfo.isReferrer()) {
                ...
                // Foreign
                // =======
                final Sample1ForeignInfo foreignInfo = localDBMeta.findForeignInfo(relationPropName);
                ...
                if (!alreadyRegisteredEntityMap.containsKey(alreadyRegisteredEntityKey)) {
                    ...
                    // - - - - - - - - - - - - - - - - - - - - - - -
                    final Sample1Entity foreignEntity = foreignInfo.getForeignDBMeta().newEntity();
                    ...
                    // - - - - - - - - - - - - - -
                    final Sample1Entity foreignEntity = alreadyRegisteredEntityMap.get(alreadyRegisteredEntityKey);
                    injectForeignEntity(localEntity, foreignPropName, foreignEntity);
                }
            } else {
                // =======
                // Referrer
                // =======
                final Sample1ReferrerInfo referrerInfo = localDBMeta.findReferrerInfo(relationPropName);

                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // Extract referrer list from current local entity and initialize it if needs.
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                List<Sample1Entity> referrerList = extractReferrerList(localEntity, referrerInfo);
                ...
                if (!alreadyRegisteredEntityMap.containsKey(alreadyRegisteredEntityKey)) {
                    ...
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    final Sample1Entity referrerEntity = relationInfo.getTargetDBMeta().newEntity();
     ...
     * @param primaryKeyMap The map of primary key. (NotNull)
     */
    protected void doLastLoopInjection(TopInfo<LOCAL_ENTITY> topInfo, Sample1HierarchyRequestElement requestElement,
                                       Sample1Entity localEntity, Map<String, Object> primaryKeyMap) {
        final Sample1ColumnInfo destinationColumnInfo = requestElement.getDestinationColumnInfo();
        if (!primaryKeyMap.containsKey(destinationColumnInfo.getColumnDbName())) {// The column is primary key!
            final Sample1HierarchySourceRow sourceRow = topInfo.getSourceRow();
            final Sample1HierarchySourceColumn sourceColumnInfo = requestElement.getSourceColumnInfo();
    ...
    protected Map<String, Object> extractTopPrimaryKeyMapFromSource(TopInfo<LOCAL_ENTITY> topInfo) {
        return extractPrimaryKeyMapFromSource(topInfo, Sample1HierarchyRequestElement.TOP_KEY);
    }

    protected Map<String, Object> extractPrimaryKeyMapFromSource(TopInfo<LOCAL_ENTITY> topInfo, String relationPath) {
        final Sample1HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Sample1HierarchySourceRow sourceRow = topInfo.getSourceRow();
        final java.util.List<Sample1HierarchyRequestElement> primaryKeyElement = request.findPrimaryKeyElement(relationPath);
        final java.util.Map<String, Object> primaryKeyMap = new java.util.LinkedHashMap<String, Object>();
        for (Sample1HierarchyRequestElement element : primaryKeyElement) {
            final Sample1HierarchySourceColumn sourcePrimaryKey = element.getSourceColumnInfo();
    ...
    protected Object extractColumnValueFromSource(Sample1HierarchySourceRow sourceRow,
            Sample1HierarchySourceColumn sourceColumn) {
        return sourceRow.extractColumnValue(sourceColumn);
    }

    @SuppressWarnings("unchecked")
    protected java.util.List<Sample1Entity> extractReferrerList(Sample1Entity entity, Sample1ReferrerInfo referrerInfo) {
        return (java.util.List<Sample1Entity>) invoke(referrerInfo.findGetter(), entity, new Object[] {});
    }
     ...
     */
    protected void injectColumnValueToDestinationIfNotNull(Sample1Entity entity, Sample1ColumnInfo columnInfo, final Object columnValue) {
        if (columnValue != null) {
            injectColumnValueToDestination(entity, columnInfo.getColumnDbName(), columnValue);
        }
    }

    protected void injectColumnValueToDestination(Sample1Entity entity, String columnDbName, final Object columnValue) {
        if (columnValue == null) {
    ...
    protected void injectColumnValueMapToDestination(Sample1Entity entity, final Map<String, Object> columnValueMap) {
    ...
    protected void injectForeignEntity(Sample1Entity entity, String foreignPropName, Sample1Entity foreignEntity) {
    ...
    protected void injectReferrerList(Sample1Entity entity, Sample1ReferrerInfo referrerInfo, java.util.List<Sample1Entity> referrerList) {
        invoke(referrerInfo.findSetter(), entity, new Object[] { referrerList });
    }

    protected void injectForeignPrimaryKey(Sample1Entity foreignEntity, Map<String, Object> foreigPrimaryKeyMap) {
        injectColumnValueMapToDestination(foreignEntity, foreigPrimaryKeyMap);
    }

    protected void injectReferrerPrimaryKey(Sample1Entity referrerEntity, Map<String, Object> referrerPrimaryKeyMap) {
        injectColumnValueMapToDestination(referrerEntity, referrerPrimaryKeyMap);
    }

    protected void injectLocalForeignKey(TopInfo<LOCAL_ENTITY> topInfo, Sample1Entity localEntity, Sample1ForeignInfo foreignInfo, String foreignRelationPath) {
        final Sample1HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Object> foreignPrimaryKeyMap = extractPrimaryKeyMapFromSource(topInfo, foreignRelationPath);
        final List<Sample1HierarchyRequestElement> primaryKeyElementList = request.findPrimaryKeyElement(foreignRelationPath);
        final Map<String, Object> localForeignKeyMap = new HashMap<String, Object>();
        for (Sample1HierarchyRequestElement foreignElement : primaryKeyElementList) {
            final String foreignPrimaryKeyColumnName = foreignElement.getDestinationColumnInfo().getColumnDbName();
            final Sample1ColumnInfo localForeignKeyInfo = foreignInfo.findLocalByForeign(foreignPrimaryKeyColumnName);
    ...
    protected void injectReferrerForeignKey(TopInfo<LOCAL_ENTITY> topInfo, Sample1Entity referrerEntity,
            Sample1ReferrerInfo referrerInfo, String localRelationPath) {
        final Sample1HierarchyRequest<LOCAL_ENTITY> request = topInfo.getHierarchyRequest();
        final Map<String, Object> localPrimaryKeyMap = extractPrimaryKeyMapFromSource(topInfo, localRelationPath);
        final List<Sample1HierarchyRequestElement> primaryKeyElementList = request.findPrimaryKeyElement(localRelationPath);
        final Map<String, Object> referrerForeignKeyMap = new HashMap<String, Object>();
        for (Sample1HierarchyRequestElement localElement : primaryKeyElementList) {
            final String localPrimaryKeyName = localElement.getDestinationColumnInfo().getColumnDbName();
            final Sample1ColumnInfo referrerForeignKeyInfo = referrerInfo.findReferrerByLocal(localPrimaryKeyName);
    ...
    //                                                                         ===========
    @SuppressWarnings("unchecked")
    protected LOCAL_ENTITY newLocalEntity(final Sample1DBMeta destinationDBMeta) {
    ...
    //                                                                          Info Class
    //                                                                          ==========
    protected static class TopInfo<LOCAL_ENTITY extends Sample1Entity> {
        private Sample1HierarchyRequest<LOCAL_ENTITY> hierarchyRequest;

        private Sample1HierarchySourceRow sourceRow;

        private Sample1Entity localEntity;

        private Map<String, Sample1Entity> alreadyRegisteredEntityMap;

        public Sample1HierarchySourceRow getSourceRow() {
            return sourceRow;
        }

        public void setSourceRow(Sample1HierarchySourceRow sourceRow) {
            this.sourceRow = sourceRow;
        }

        public Sample1Entity getLocalEntity() {
            return localEntity;
        }

        public void setLocalEntity(Sample1Entity localEntity) {
            this.localEntity = localEntity;
        }

        public Map<String, Sample1Entity> getAlreadyRegisteredEntityMap() {
            return alreadyRegisteredEntityMap;
        }

        public void setAlreadyRegisteredEntityMap(Map<String, Sample1Entity> alreadyRegisteredEntityMap) {
            this.alreadyRegisteredEntityMap = alreadyRegisteredEntityMap;
        }

        public Sample1HierarchyRequest<LOCAL_ENTITY> getHierarchyRequest() {
            return hierarchyRequest;
        }

        public void setHierarchyRequest(Sample1HierarchyRequest<LOCAL_ENTITY> hierarchyRequest) {
